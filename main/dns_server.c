...
static const char *TAG = "captative_portal";
...
static int parse_dns_request(char *req, size_t req_len, char *dns_reply, size_t dns_reply_max_len, dns_server_handle_t h)
{
    ...
    ESP_LOGI(TAG, "DNS query with header id: 0x%X, flags: 0x%X, qd_count: %d",
    ...
    for (int qd_i = 0; qd_i < qd_count; qd_i++) {
        ...
        if (qd_type == QD_TYPE_A) {
            ...
            ESP_LOGI(TAG, "Answer with PTR offset: 0x%" PRIX16 " and IP 0x%" PRIX32, ntohs(answer->ptr_offset), ip.addr);

...
float getTemperature() { return dht_data.temp; }
...
int getSignalLevel( int usTimeOut, bool state )
{
	...
	int uSec = 0;
	while( gpio_get_level(dht_data.pin)==state ) {
...
int readDHT()
{
	...
	gpio_set_direction( dht_data.pin, GPIO_MODE_OUTPUT );

	// pull down for 3 ms for a smooth and nice wake up 
	gpio_set_level( dht_data.pin, 0 );
	esp_rom_delay_us( 3000 );			

	// pull up for 25 us for a gentile asking for data
	gpio_set_level( dht_data.pin, 1 );
	esp_rom_delay_us( 25 );

	gpio_set_direction( dht_data.pin, GPIO_MODE_INPUT );		// change to input mode
...
//	ESP_LOGI( TAG, "Response = %d", uSec );
	...
	dht_data.temp = dhtData[2] & 0x7F;	
	dht_data.temp *= 0x100;				// >> 8
	dht_data.temp += dhtData[3];
	dht_data.temp /= 10;
